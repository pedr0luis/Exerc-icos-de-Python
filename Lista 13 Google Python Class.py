...
def fim_igual(words):
  ...
  for c in range (len(words)):
      palavra=words[c]
      if len(palavra)>=2 and palavra[0]==palavra[len(palavra)-1]:
        R=R+1
  return R 
...
def x_antes(words):
    ...
    listaI = []
    listaII = []
    listaI.append(words[0])
    listaI.append(words[1])
    listaI.sort(reverse=False)
    for c in range (2,len(words)):
       listaII.append(words[c])
    listaII.sort(reverse=False)
    for c in range (0,3):
       if c>2:
           listaI.append(listaI[c])
       else:
           listaI.append(listaII[c])
    return listaI 
