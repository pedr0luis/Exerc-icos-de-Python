...
def near_ten(n):
  ...
  if resto==0 or resto==1:
    ...
    return False
  elif resto>=3 and resto<=5:
    return False
  else:
    for c in range (20):
      if resto*c<=12 and resto*c>=8:
        return True
        break
      elif resto*c>=13:    
        return False
...
def double_char(s):
  palavra =''
  for c in range (len(s)):palavra =palavra+s[c]+s[c]
  return palavra
...
def count_code(s):
  ...
  for c in range (len(s)-3):
    if s[c]=='c' and s[c+1]=='o' and s[c+3]=='e':
      code=code+1
  return code
...
def soma_na_lista(n, lista):
  ...
  for c in range (len(lista)):
    for C in range (len(lista)):
        if lista[c]!=lista[C] and lista[c]+lista[C]==n:
            return True
            break
  return False
